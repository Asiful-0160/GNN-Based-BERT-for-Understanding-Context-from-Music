#!/usr/bin/env python3
"""
Script 53 — Formal Task-3 M3 Phase-B Joint Fine-Tuning
======================================================

Pipeline Plan v6 Phase B:
    - initialize from selected T1-B encoder
    - initialize from selected A4 GAT/G2 encoder
    - initialize fusion from best M3 Phase-A checkpoint
    ...
    - epochs = 5
    - batch = 16
    - select by validation macro mean per-label AP

No separate preflight is created. Required integrity checks are embedded here.
...
MODEL_DIR = (
    ROOT / "models/task3_candidates"
)

RUN_DIR = (
    ROOT / "results/runs/task3"
)
...
EXPECTED_PHASEA_RESULTS_FREEZE_SHA256 = (
...
M2_VAL_MACRO_AP = 0.18752693895679443

M3_PHASEA_VAL_MACRO_AP = 0.830729168310


EXPECTED_BERT_TOP2_TRAINABLE = 14_175_744
EXPECTED_GNN_ENCODER_TRAINABLE = 101_056
EXPECTED_M3_TRAINABLE = 366_356

EXPECTED_TOTAL_TRAINABLE = (
    EXPECTED_BERT_TOP2_TRAINABLE
    + EXPECTED_GNN_ENCODER_TRAINABLE
    + EXPECTED_M3_TRAINABLE
)
...
def verify_output_protection() -> None:
    ...
    banner(
        "1. FORMAL M3 PHASE-B OUTPUT PROTECTION"
    )
    ...
    if FAILURES:
        ...
        raise RuntimeError(
            "Refusing to rerun formal M3 Phase B"
        )


# =============================================================================
# 6. MINIMAL INTEGRITY CHECKS
# =============================================================================

def verify_dependencies() -> None:

    banner(
        "2. REQUIRED FROZEN DEPENDENCIES"
    )
    ...
    check(
        ...
        is True,
        "Task-3 Phase A is formally complete",
    )
    ...
    check(
        ...
        is False,
        "Phase-A freeze records TEST unused",
    )
    ...
    if FAILURES:
        ...
        raise RuntimeError(
            "Frozen Phase-B dependency check failed"
        )


# =============================================================================
# 7. GRAPH MODEL — EXACT SELECTED A4
# =============================================================================
...
class A4GAT(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        # Original Task-2 classifier.
        # Loaded for exact checkpoint parity,
        # but unused and frozen in Task 3.
...
# =============================================================================
# 8. M3 FUSION — EXACT PHASE-A ARCHITECTURE
# =============================================================================
...
def build_phase_b_model(
    ...
    banner(
        "3. RECONSTRUCT M3 PHASE-B MODEL"
    )

    # -------------------------------------------------------------------------
    # BERT — source = selected T1-B
    # -------------------------------------------------------------------------
    ...
    check(
        ...
        "T1-B BERT strict load: zero missing keys",
    )

    check(
        len(incompatible.unexpected_keys) == 0,
        "T1-B BERT strict load: zero unexpected keys",
    )

    # -------------------------------------------------------------------------
    # A4 GAT — source = selected Task-2 A4
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # Fusion — source = best M3 Phase A
    # -------------------------------------------------------------------------

    fusion = M3Fusion()

    phase_a_checkpoint = load_checkpoint(
        M3_PHASEA_CHECKPOINT
    )

    check(
        phase_a_checkpoint.get(
            "variant"
        )
        == "M3",
        "Phase-A source variant = M3",
    )
    ...
    # Pipeline-v6 Phase B:
    #
    # last TWO BERT encoder layers.
    # -------------------------------------------------------------------------

    for layer_index in (10, 11):
    ...
    # -------------------------------------------------------------------------
    # Pipeline-v6 says "+ GNN".
    #
    # Train the selected GNN encoder:
    # node projection + all three GAT layers.
    #
    # Old Task-2 classifier is not part of Task-3 pathway.
    # -------------------------------------------------------------------------
    ...
    print(
        f"Trainable GNN encoder: "
        f"{gnn_trainable:,}"
    )

    print(
        f"Trainable M3 fusion: "
        f"{fusion_trainable:,}"
    )
    ...
    check(
        ...
        == EXPECTED_GNN_ENCODER_TRAINABLE,
        (
            "full A4 GNN encoder is "
            "trainable: 101,056 params"
        ),
    )
    ...
    check(
        ...
        == EXPECTED_TOTAL_TRAINABLE,
        (
            "total Phase-B trainable params "
            "= 14,643,156"
        ),
    )
    ...
    for layer_index in range(10):
        ...
        check(
            ...
            (
                f"BERT encoder layer "
                f"{layer_index} remains frozen"
            ),
        )
    ...
    check(
        ...
        (
            "old A4 20-label classifier "
            "remains frozen and unused"
        ),
    )
...
def load_g2_graph(
    ...
    if x.shape != (9, 140):
        ...
        raise RuntimeError(
            f"{graph_path.name}: "
            f"x shape={x.shape}, expected (9,140)"
        )
    ...
    if not (
        ...
        raise RuntimeError(
            f"{graph_path.name}: "
            "G2 directed-edge count outside 16..34"
        )
    ...
    if not np.isclose(
        ...
        raise RuntimeError(
            f"{graph_path.name}: "
            "G2 similarity threshold changed"
        )
...
# =============================================================================
# 12. BUILD DEVELOPMENT DATA — TRAIN+VAL ONLY
# =============================================================================

def load_development_data():

    banner(
        "4. LOAD LIVE PHASE-B INPUTS — TRAIN+VAL ONLY"
    )
    ...
    check(
        ...
        "token manifest schema is unchanged",
    )
    ...
    check(
        ...
        (
            "Phase-B development plan "
            "contains zero TEST rows"
        ),
    )

    # -------------------------------------------------------------------------
    # Align graph manifest.
    # -------------------------------------------------------------------------
    ...
    targets = target_table.to_pandas()
    ...
    check(
        ...
        (
            "development token and "
            "target track-ID sets match"
        ),
    )
    ...
    # -------------------------------------------------------------------------
    # Extract TRAIN+VAL token rows.
    #
    # bert_tokens.npz contains the immutable whole-dataset token cache,
    # but only TRAIN/VAL indices are retained and supplied to the model.
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # Load ONLY TRAIN+VAL raw G2 graph files.
    # Preload once to avoid thousands of repeated small-file reads.
    # -------------------------------------------------------------------------
...
def compute_pos_weight(
    ...
    check(
        ...
        (
            ...
            "matches frozen Tasks 1-3 vector"
        ),
    )
...
# =============================================================================
# 16. PHASE-B STATE CAPTURE
# =============================================================================

def capture_phase_b_state(
    model: M3PhaseBModel,
) -> dict[str, Any]:
...
def build_optimizer(
    ...
    check(
        ...
        EXPECTED_M3_TRAINABLE,
        (
            "optimizer fusion group contains "
            "all M3 parameters"
        ),
    )
    ...
    check(
        len(parameter_ids)
        ==
        len(set(parameter_ids)),
        (
            "optimizer contains no "
            "duplicate parameter"
        ),
    )
    ...
    print(
        "LR: 5e-6 for both groups"
    )

    print(
        "BERT top-2 weight decay: 0.01"
    )

    print(
        "GNN + fusion weight decay: 0.0001"
    )
...
def run_training(
    ...
    # -------------------------------------------------------------------------
    # Important parity check:
    #
    # live T1-B + live A4 + Phase-A M3 fusion should reproduce
    # the accepted Phase-A validation result BEFORE joint updates.
    # -------------------------------------------------------------------------
    ...
    print(
        ...
        f"{M3_PHASEA_VAL_MACRO_AP:.12f}"
    )

    print(
        f"  live reconstructed AP = "
    ...
    if FAILURES:
        ...
        raise RuntimeError(
            "Live Phase-A reconstruction parity failed"
        )
    ...
    # -------------------------------------------------------------------------
    # Restore best epoch and independently re-evaluate it.
    # -------------------------------------------------------------------------

    restore_phase_b_state(
        model,
        best_state,
    )
    ...
    if FAILURES:
        ...
        raise RuntimeError(
            "Best Phase-B state revalidation failed"
        )
...
def save_result(
    ...
    banner(
        "8. SAVE FORMAL M3 PHASE-B RESULT"
    )
    ...
    checkpoint = {
        ...
        "trainable_contract": {
            ...
            "gnn":
                (
                    "full A4 encoder: "
                    "node_projection + gat1 + gat2 + gat3"
                ),
            ...
            "fusion":
                "all M3 Phase-A fusion parameters",
...
def main() -> int:
    ...
    print(
        "  old A4 classifier:  FROZEN / UNUSED"
    )
    ...
    try:
        ...
        if (
            ...
            print(
                "No result from this run "
                "should be accepted."
            )

            print(
                "Pipeline v6 permits gradient "
                "accumulation x2 if OOM."
            )

            print(
                "Stop and review before rerunning."
            )
    ...
    print(
        f"  delta       = "
        f"{best['macro_ap'] - M3_PHASEA_VAL_MACRO_AP:+.8f}"
    )
    ...
    print(
        f"  BERT top 2 = "
        f"{EXPECTED_BERT_TOP2_TRAINABLE:,}"
    )

    print(
        f"  GNN encoder = "
        f"{EXPECTED_GNN_ENCODER_TRAINABLE:,}"
    )

    print(
        f"  M3 fusion   = "
        f"{EXPECTED_M3_TRAINABLE:,}"
    )
    ...
    print(
        "Review Script 53 output "
        "before running M4 Phase B."
    )

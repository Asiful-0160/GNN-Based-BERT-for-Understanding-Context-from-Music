#!/usr/bin/env python3
"""
Script 51 — Formal Task-3 M4 Cross-Attention Phase-A Training
==============================================================
...
Script 50:
    Formal M3 Phase-A PASS

Frozen Task-3 design SHA256:
    0a9d50413b65aeed58d02d6a2dff70d99363ae25fee115d30bc9749f54e1c9ef

M4 — Pipeline Plan v6
---------------------
BERT final-layer token sequence:
    (112, 768)
        -> Linear(768,256)
        -> K,V

A4 GAT/G2 graph embedding:
    128
        -> Linear(128,256)
        -> Q
...
Cross-attention:
    ...
    Q = graph_prime.unsqueeze(1)
    K = projected BERT sequence
    V = projected BERT sequence

    key_padding_mask = ~attention_mask.bool()

Fusion:
    concat(graph_prime, attended)
        -> 512
        -> Linear(512,256)
        -> ReLU
        -> Linear(256,20)

No additional M4 fusion dropout is added because it is not present in
the frozen Script-49 design.
...
Only M4 fusion parameters are trained:
    629,524
...
Selection:
    highest validation macro mean per-label Average Precision

STRICT TEST POLICY
------------------
TEST representations:      NOT PRESENT in development caches
TEST target values:        NOT LOADED
...
After Script 51:
    Do NOT run Phase B immediately.
    First freeze/review both Phase-A results.
"""
...
# -----------------------------------------------------------------------------
# Accepted Script-50 formal M3 run
# -----------------------------------------------------------------------------
...
def save_checkpoint_atomic(
    ...
    if temp.exists():
        ...
        raise RuntimeError(
            f"Temporary checkpoint already exists: {temp}"
        )
...
# =============================================================================
# 6. EXACT FROZEN M4 ARCHITECTURE
# =============================================================================

class M4CrossAttention(
    nn.Module
):
...
def verify_no_prior_formal_run() -> None:
    ...
    if FAILURES:
        ...
        raise RuntimeError(
            "Formal M4 output collision detected"
        )


# =============================================================================
# 8. VERIFY FROZEN SCRIPT-49 DESIGN
# =============================================================================
...
def verify_design() -> dict[str, Any]:
    ...
    if FAILURES:
        ...
        raise RuntimeError(
            "Task-3 design artifact missing"
        )
...
# =============================================================================
# 9. VERIFY ACCEPTED SCRIPT-50 M3 RESULT
# =============================================================================

def verify_m3_phase_a_result() -> None:

    banner(
        "3. ACCEPTED M3 PHASE-A RESULT"
    )
    ...
    for path in expected:
        ...
        check(
            path.is_file(),
            (
                "accepted Script-50 artifact exists: "
    ...
    if FAILURES:
        ...
        raise RuntimeError(
            "Accepted M3 Phase-A artifacts missing"
        )
    ...
    check(
        ...
        (
            "accepted M3 VAL macro AP "
            "matches Script-50 result"
        ),
    )
    ...
    check(
        ...
        is True,
        (
            "accepted M3 result requires "
            "Phase B under frozen gate"
        ),
    )
    ...
    if FAILURES:
        ...
        raise RuntimeError(
            "Accepted M3 Phase-A validation failed"
        )


# =============================================================================
# 10. VERIFY ALL FROZEN DEPENDENCIES
# =============================================================================
...
def verify_frozen_dependencies() -> None:
    ...
    if FAILURES:
        ...
        raise RuntimeError(
            "Frozen dependency identity validation failed"
        )
...
def load_aligned_development_data():
    ...
    check(
        ...
        (
            ...
            "contain zero TEST rows"
        ),
    )

    # -------------------------------------------------------------------------
    # Load TRAIN+VAL target VALUES only.
    # TEST target values are not returned.
    # -------------------------------------------------------------------------
    ...
    check(
        ...
        (
            "loaded target values "
            "contain TRAIN+VAL only"
        ),
    )
    ...
    # -------------------------------------------------------------------------
    # Memory-map frozen representations.
    # -------------------------------------------------------------------------
...
def obtain_pos_weight(
    ...
    check(
        ...
        (
            "stored T1-B and A4 "
            "pos_weight vectors are identical"
        ),
    )
    ...
    if FAILURES:
        ...
        raise RuntimeError(
            "TRAIN-only pos_weight validation failed"
        )
...
def build_loaders(
    ...
    banner(
        "7. FORMAL TRAIN / VAL DATALOADERS"
    )
...
def build_model(
    ...
    check(
        tuple(
            model.text_projection.weight.shape
        )
        == (
            256,
            768,
    ...
    check(
        tuple(
            model.graph_projection.weight.shape
        )
        == (
            256,
            128,
    ...
    check(
        ...
        == 0,
        (
            "M4 contains no extra "
            "Dropout module"
        ),
    )

    # Encoders must not exist in Phase A.
    module_names = [
        name.lower()
        for name, _
    ...
    if FAILURES:
        ...
        raise RuntimeError(
            "Formal M4 architecture validation failed"
        )
...
def train_one_epoch(
    ...
    for (
        ...
        if not torch.isfinite(
            ...
            raise RuntimeError(
                "Non-finite TRAIN logits detected"
            )
...
# =============================================================================
# 18. FORMAL TRAINING LOOP
# =============================================================================

def run_formal_training(
    model: M4CrossAttention,
    train_loader: DataLoader,
    val_loader: DataLoader,
    ...
    if (
        ...
        raise RuntimeError(
            "No valid best M4 checkpoint produced"
        )
...
# =============================================================================
# 19. BEST-CHECKPOINT REVALIDATION
# =============================================================================
...
def revalidate_best_checkpoint(
    ...
    check(
        ...
        EXPECTED_M3_CHECKPOINT_SHA256,
        (
            "checkpoint records accepted "
            "Script-50 M3 dependency"
        ),
    )
    ...
    if FAILURES:
        ...
        raise RuntimeError(
            "Best M4 checkpoint revalidation failed"
        )
...
def write_run_artifacts(
    ...
    if FAILURES:
        ...
        raise RuntimeError(
            "Formal M4 run-output collision"
        )
    ...
    if epoch_temp.exists():

        raise RuntimeError(
            f"Temporary epoch file already exists: "
            f"{epoch_temp}"
        )
...
def main() -> int:
    ...
    banner(
        "SCRIPT 51 — FORMAL TASK-3 M4 CROSS-ATTENTION PHASE-A"
    )
    ...
    try:
        ...
        # ---------------------------------------------------------------------
        # Never silently rerun a formal M4 seed-42 experiment.
        # ---------------------------------------------------------------------

        verify_no_prior_formal_run()

        # ---------------------------------------------------------------------
        # Exact frozen Script-49 design.
        # ---------------------------------------------------------------------

        verify_design()

        # ---------------------------------------------------------------------
        # Enforce formal sequence M3 Phase A -> M4 Phase A.
        # ---------------------------------------------------------------------

        verify_m3_phase_a_result()

        # ---------------------------------------------------------------------
        # Frozen cache/checkpoint identities.
        # ---------------------------------------------------------------------
        ...
        # ---------------------------------------------------------------------
        # Re-load selected checkpoint and reproduce validation metric.
        # ---------------------------------------------------------------------
    ...
    print(
        "  Task 3 / M4 / Cross-Attention / Phase A"
    )
    ...
    print(
        "  Do NOT begin Phase B yet."
    )

    print(
        "  First freeze/review both "
        "Phase-A results."
    )
    ...
    print(
        "M4 trainable parameters:       629,524"
    )
    ...
    print(
        "Review Script 51 before freezing "
        "Task-3 Phase-A results."
    )

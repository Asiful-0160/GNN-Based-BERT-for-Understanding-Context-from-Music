...
Published work also uses MusicCaps for closely related audio--language problems. Srivatsan et al.~\cite{rgmc} evaluate retrieval-guided music captioning on MusicCaps, combining audio features with a retrieved caption to generate descriptions. Wu et al.~\cite{clamp3} evaluate text-to-music retrieval on MusicCaps in their CLaMP 3 study, including original clips/captions and modified evaluation settings; their main comparison also uses MusicCaps-Remake. These studies support MusicCaps as a relevant audio--language evaluation resource, particularly for Task 4. They do not validate our exact Top-20 label construction or graph architecture. Differences in training exposure, splits, candidate pools, and metrics prevent direct comparison with this project's frozen 514-pair TEST results. The CLaMP 3 study also discusses overlapping pretraining data, reinforcing the need to document dataset and model provenance.
...
\section{Dataset Acquisition and Preprocessing}\label{sec:data}
\subsection{Why MusicCaps supports all four tasks}
MusicCaps was selected because each example links a timestamped audio clip, a musician-written description, and an aspect list~\cite{musiccaps,musiclm}. These three views make it possible to define all four experiments on the same underlying music examples. Table~\ref{tab:taskdata} explains the role of each view. This is a deliberate task construction: the 20-label vocabulary and segment graphs are project-derived representations, not official MusicCaps classification labels or supplied graph annotations.
\begin{table}[t]\centering\small
\caption{Role of MusicCaps in the four project tasks.}\label{tab:taskdata}
\begin{tabular}{@{}p{.13\linewidth}p{.79\linewidth}@{}}\toprule
Task & Dataset support and project construction\\\midrule
1 & Captions provide BERT inputs; TRAIN-selected aspects define the shared multi-label target.\\
2 & Paired audio provides mel features and segment graphs; the same aspect-derived labels supervise CNN/GNN classifiers.\\
3 & Caption and audio share a track identity, enabling aligned graph--text fusion with unchanged targets.\\
4 & Original audio--caption pairs define contrastive positives and bidirectional retrieval; descriptions also support listener judgments.\\\bottomrule
\end{tabular}
\end{table}

Using one corpus reduces confounding from changing datasets, annotation systems, or test populations between the text, audio, and fusion comparisons. Its descriptions cover audible instrumentation, tempo, mood, and recording characteristics that match the project's context-prediction objective. Ten-second clips also provide a consistent unit for fixed-length feature extraction and segment graphs. The recovered 5,140 examples exceeded the predefined 3,000-clip acquisition gate, allowing MusicCaps to remain the canonical dataset without introducing label-generated substitute captions.

The choice is suitable for a controlled course-scale study, rather than evidence that MusicCaps exhaustively represents musical context. Tasks 1--3 use an aspect-derived proxy target, with unresolved caption--label overlap; Task 4 uses the original pairing more directly. The corpus does not supply the emotion-regression targets needed for that optional extension. These boundaries motivate the shared-dataset design while limiting the claims drawn from it.

\subsection{Acquisition and canonical split}
The initial metadata contained 5,521 unique track and source IDs, with no missing captions or aspect lists. MusicCaps distributes source identifiers and timestamps, so acquisition requires retrieving the corresponding audio and extracting the annotated interval~\cite{musiccaps}. The project used yt-dlp and FFmpeg, converting accepted audio to mono 22,050-Hz PCM-16 WAV. A deterministic 200-track audit at seed 42 yielded 188 usable clips, establishing a 94\% pilot recovery rate before full acquisition.

Acquisition was treated as a validation stage rather than a download-only step. Independent decoding checked the retrieved WAV files. Some approximately 20-second downloads were investigated for temporal alignment; the intended interval lay in the final approximately ten-second region for affected artifacts. Blindly taking the first ten seconds would therefore have risked mismatching the audio with its caption. Full acquisition initially recovered 4,847 clips and controlled recovery increased this to 5,140. The remaining 381 clips were excluded, yielding 93.10\% usable data. Table~\ref{tab:data} records the final dataset boundary. Exclusions are retained as a coverage limitation, not replaced with synthesized examples.
\begin{table}[t]
...
Preprocessing follows a fixed dependency order: freeze the usable track set and split, derive vocabulary and token length from TRAIN, generate aligned audio/text/graph caches, and verify their identities before training. This separates decisions learned from data from deterministic feature extraction. The shared track ID is the alignment key across modalities, preventing an apparently valid feature matrix from being paired with the wrong caption or label row.
...
Experiments were conducted in Ubuntu 22.04 running through Windows Subsystem for Linux (WSL) on a Windows host. The workstation used an Intel Core i7-14700K processor (3.40 GHz), 64.0 GB installed RAM (63.8 GB usable), and an NVIDIA GeForce RTX 4080 SUPER with 16 GB graphics memory. The host was a 64-bit operating system on an x64-based processor. These hardware and operating-system details document the execution environment; they do not substitute for the unavailable package, driver, and CUDA version records.
...
The preprocessing verification stage freezes dataset, feature, and target artifacts through SHA256 identities. Final target-dependent supervised TEST evaluation begins only after model and threshold selection. Task 4 locks design, selection, multiseed validation, and pre-TEST protocol before final retrieval evaluation. Subsequent analyses reuse saved outputs. Aborted stale-cache or numerical revalidation attempts are retained as provenance; implementation corrections during threshold freezing, contrastive fine-tuning, and multiseed validation preceded the relevant TEST access and did not justify changing the frozen scientific choices.

Supervised seed uncertainty uses sample SD ($\mathrm{ddof}=1$); Task-4 seed uncertainty uses population SD ($\mathrm{ddof}=0$). Human pooled and item SDs use sample SD. These quantities describe different sources of variability and are not confidence intervals. Fusion and contrastive seeds may condition on shared supervised initialization; they do not establish variability of independently rebuilt end-to-end pipelines. Final BERT evaluation uses FP16 autocast, whereas the other supervised models use FP32. Elapsed training time, complete package versions, and Git commit are unavailable in the provided evidence.
...
\textbf{Provenance coverage.} The independent audit cannot certify nine source files covering parts of acquisition/recovery, target preparation, and reporting. Git commit, complete software versions, and some training hyperparameters are unavailable. The Task-3 comparator in Task-4 consolidation is report-level rather than fully linked through the Task-4 hash chain; the separate Task-3 final JSON independently supplies the same value. Supplied case-study assets are available at result level, but this does not establish source-level verification of the reporting stages.
...
Table~\ref{tab:audit} summarizes the implementation audit. ``Not verified'' marks a source-evidence limit, not a confirmed failure. Minor corrections concern sparse-graph design rationale, numerical revalidation, seed interpretation, reporting-only thresholds, exploratory error analysis, prototype-readout terminology, and final-result provenance. An early graph-threshold probe was superseded by a larger TRAIN-only structural study.
\begin{table}[!ht]\centering\small
\caption{Implementation audit summary across 76 source files.}\label{tab:audit}
\begin{tabular}{@{}lr@{}}\toprule

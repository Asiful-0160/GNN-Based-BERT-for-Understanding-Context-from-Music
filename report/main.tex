...
\newcommand{\pendingfigure}[1]{\IfFileExists{figures/#1}{\includegraphics[width=\linewidth]{figures/#1}}{\par\small\textit{Requested source figure not yet supplied.}\par}}
\begin{document}
\onecolumn
\begin{titlepage}
\centering
...
All four assignment stages are implemented~\cite{guideline}. The independent audit documents the controlled experimental chain supporting the reported comparisons~\cite{audit}.
...
For qualitative evaluation, ten query indices and seed 42 were fixed before inspecting TEST retrieval. The saved similarity matrix supplies caption-to-audio top-three results; the listening study uses top-one clips for those same queries. The supplied HTML asks listeners to read each caption, hear the complete clip, and independently assign one rating: 1 no match, 2 weak match, 3 moderate match, 4 good match, or 5 excellent match. It requires a listener ID and all ten ratings before CSV export, but does not technically enforce complete playback. Five listeners provide a complete 50-rating design.
...
\textbf{Provenance coverage.} The independent audit cannot certify the full source of nine scripts (07--12, 18, 62, 63). Git commit, full environment, and some training hyperparameters are unavailable. The Task-3 comparator in Task-4 consolidation is report-level rather than fully linked through the Task-4 hash chain; the separate Task-3 final JSON independently supplies the same value. Supplied case-study assets are available at result level, but this does not retroactively certify Scripts 62--63. The requested t-SNE and complete ten-query top-three retrieval panels are not present in the supplied export.
...
Table~\ref{tab:audit} summarizes the 76-script audit. ``Not verified'' marks a source-evidence limit, not a confirmed failure. The nine minor-fix entries are 27, 57--59, 61, 68, 70, 73, and 76; Script 25 is redundant. The audit contains the full per-script dispositions~\cite{audit}.
...
The original brief requests t-SNE and ten caption-to-top-three qualitative examples. Figure~\ref{fig:tsne} reserves the former supplied-artifact location; Figure~\ref{fig:qualitative} reserves the latter. No coordinates, ranks, retrieved identities, or missing audio are reconstructed from aggregate metrics. Query captions and rating summaries alone cannot supply the absent top-three retrieval results.
\begin{figure}[!ht]\centering
\pendingfigure{fig_tsne_task3.png}
\caption{Reserved Task-3 t-SNE visualization from frozen fusion embeddings. Interpretation awaits the actual supplied figure and its label/color specification.}\label{fig:tsne}
\end{figure}
\begin{figure}[!ht]\centering
\pendingfigure{fig_task4_qualitative_top3.png}
\caption{Reserved qualitative panel for the ten pre-frozen caption queries and their seed-42 top-three audio retrievals. Actual retrieval examples must come from the frozen Script-74 export.}\label{fig:qualitative}
\end{figure}
